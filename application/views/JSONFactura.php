{
	"DatosGenerales": {
		"Version": "3.3",
		"Usuario": "PruebasTimbrado",
		"Password": "@Notiene1",
		"SellaCFDI": "true",
		"TimbraCFDI": "true",
		"CSD": "",
		"LlavePrivada": "",
		"CSDPassword": "12345678a",
		"GeneraPDF": "true",
		"Logotipo": "",
		"CFDI": "Factura",
		"OpcionDecimales": "1",
		"NumeroDecimales": "2",
		"TipoCFDI": "Ingreso"
	},
	"Encabezado": {
		"Emisor": {
			"RFC": "AAA010101AAA",
			"NombreRazonSocial": "BÓVEDAS MULTIBLINDADAS S.A. DE C.V.",
			"RegimenFiscal": "601 - General Ley de las Personas Morales",
			"Direccion": [{
				"Calle": "<?=$datosSucursal['calle']?>",
				"NumeroExterior": "<?=$datosSucursal['numero']?>",
				"NumeroInterior": "<?=$datosSucursal['numeroInterior']?>",
				"Colonia": "<?=$datosSucursal['nombreRegion']?>",
				"Localidad": "<?=$datosSucursal['nombreEstado']?>",
				"Municipio": "<?=$datosSucursal['nombreMunicipio']?>",
				"Estado": "<?=$datosSucursal['nombreEstado']?>",
				"Pais": "México",
				"CodigoPostal": "<?=$datosSucursal['codigoPostal']?>"
			}]
		},
		"Receptor": {
			"RFC": "SSF1103037F1",
			"NombreRazonSocial": "<?=$datosCliente['razonSocial']?>",
			"UsoCFDI": "P01 - Por Definir",
			"Direccion": {
				"Calle": "<?=$datosCliente['calle']?>",
				"NumeroExterior": "<?=$datosCliente['numero']?>",
				"NumeroInterior": "<?=$datosCliente['numeroInterior']?>",
				"Colonia": "<?=$datosCliente['nombreRegion']?>",
				"Localidad": "<?=$datosCliente['nombreEstado']?>",
				"Municipio": "<?=$datosCliente['nombreMunicipio']?>",
				"Estado": "<?=$datosCliente['nombreEstado']?>",
				"Pais": "México",
				"CodigoPostal": "<?=$datosCliente['codigoPostal']?>"
			}
		},
		"Fecha": "<?=date("Y-m-d")."T".date("H:i:s")?>",
		"Serie": "AB",
		"Folio": "1",
		"MetodoPago": "PUE - Pago en una sola exhibición",
		"FormaPago": "99-Por Definir",
		"Moneda": "MXN",
		"LugarExpedicion": "06470",
		"SubTotal": "100.00",
		"Total": "116"
	},
	"Conceptos": [{
		"Cantidad": "1",
		"CodigoUnidad": "E48",
		"Unidad": "Servicio",
		"CodigoProducto": "84111506",
		"Producto": "Timbres de Facturación",
		"PrecioUnitario": "100",
		"Importe": "100",
		"Impuestos": [{
			"TipoImpuesto": "1",
			"Impuesto": "2",
			"Factor": "1",
			"Base": "100",
			"Tasa": "0.160000",
			"ImpuestoImporte": "16"
			}]
	}]
}
